...
export default function GrowCalculator() {
  ...
  const [friendBoost, setFriendBoost] = useState(false);
  ...
  return (
    <div className="p-4 max-w-xl mx-auto space-y-4 bg-white dark:bg-gray-800 rounded shadow">
      <h1 className="text-2xl font-bold text-gray-800 dark:text-white">Grow a Garden Calculator</h1>
      ...
      <div>
        <label className="font-semibold flex items-center gap-2 text-gray-700 dark:text-white cursor-pointer">
          <input
            type="checkbox"
            className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
            checked={friendBoost}
            onChange={e => setFriendBoost(e.target.checked)}
          />
          Friend Boost
        </label>
      </div>
      ...
      {result && (
        <div className="mt-4 p-4 border rounded bg-gray-100 dark:bg-gray-700 border-gray-300 dark:border-gray-600">
          ...
          <p className="text-center text-3xl">
            <span className="text-green-600 font-bold">{result.value.toLocaleString()}</span>
          </p>
